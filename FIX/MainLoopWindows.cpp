...
public:
  ...
  ~PipeEvent() override {
    if (m_monitor_thread.joinable()) {		
      ...
      // Keep trying to cancel ReadFile() until the thread exits (MSVC style).
      do {
        CancelIoEx(m_handle, /*lpOverlapped=*/NULL);
      } while (WaitForSingleObject(m_monitor_thread.native_handle(), 1) ==
               WAIT_TIMEOUT);
#else
      // On MinGW/UCRT64, simply cancel the operation once and wait for it to complete.
      CancelIoEx(m_handle, /*lpOverlapped=*/NULL);
#endif
...
private:
  ...
  std::mutex m_mutex; // Мутекс для синхронизации состояний событий
};
